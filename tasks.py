...
@task
def conform(c):
    c.run("isort datta tests ./docs/source/conf.py setup.py -m 3 -l 120 --up --tc --lbt 0")
    c.run("black datta --line-length=120")
...
@task
def lint(c):
    c.run("isort datta tests ./docs/source/conf.py setup.py -m 3 -l 120 --up --tc --lbt 0 --check-only")
    c.run("black datta --line-length=120 --check")
    c.run("black tests --line-length=120 --check")
    c.run("black setup.py --line-length=120 --check")

    c.run("flake8 datta --count --select=E9,F63,F7,F82 --show-source --statistics")
    c.run("flake8 tests --count --select=E9,F63,F7,F82 --show-source --statistics")
    c.run(
        "flake8 datta --count --ignore=F811,F405,F403,F401,E203,E731,C901,W503 "
...
def docs(c):
    ...
    os.mkdir(api_docs)
    c.run("sphinx-apidoc datta --separate --module-first --no-toc --force --output-dir {}".format(api_docs))
