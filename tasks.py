...
@task
def conform(c):
    c.run("isort datta tests ./docs/source/conf.py setup.py -m 3 -l 88 --up --tc --lbt 0")
...
@task
def lint(c):
    c.run("isort datta tests ./docs/source/conf.py setup.py -m 3 -l 88 --up --tc --lbt 0 --check-only")
